...
标识符区分大小写，游戏注册符号统一使用小写 snake_case。
...
地图 action 可调用当前会话注册的任意 StoryCommand，不限于场景指令。once 地图事件只在 command 成功后提交；世界触发会在派发一次性 command 前记录完成状态，防止换图递归触发。
