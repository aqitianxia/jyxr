...
- 当同一角色实例可由多个 `CharacterDefinition` 模板创建时，若以后需要把既有实例从一个模板升级为另一个模板，应建立独立的“角色转化/重建”用例，不能让 `join` 隐式完成。例如低级程英升级为高级模板应走该用例。
...
- `CharacterProjection.EffectiveTalents` / `CharacterInstance.HasEffectiveTalent(...)` 当前实际表达“已学习或经 Affix 授予的候选天赋闭包”：被其他天赋通过 `ReplaceTalentIds` 替换后，只屏蔽其 Affix 效果，仍保留在该集合中并继续满足天赋存在性判断。这个行为符合当前设计，但 `Effective` 命名容易让人误以为被替换天赋已从集合移除；后续应将其重命名为能表达候选/可判断语义的名称，或显式拆分“候选天赋”和“实际贡献效果的天赋”两套查询，避免内容条件和定制 handler 误用。
...
- 战斗 AI 当前只按技能 definition 的几何范围生成主要目标，再基于这组主要目标筛选和评分候选；它不会执行技能效果计划来解析 `all_units`、`all_enemies`、`all_allies` 等 selector 产生的真实作用目标，也没有评价施加 Buff、移除状态、控制行动条等非伤害收益。因此，当技能定义的几何影响范围与效果执行时的真实影响范围不一致时，AI 可能直接丢弃本可使用的技能，或按错误的目标和收益评分；飞星术这类“对自己施放、实际影响全场”的攻击型状态绝技就是当前实例。后续应让候选生成与评分共用可预览的技能执行计划：先解析实际作用单位，再汇总伤害、治疗、状态、资源与行动节奏收益，避免继续按特殊 selector 或具体技能名称补分支。
- 战斗 AI 候选计算已完成第一轮无行为变化优化：每次决策只准备一次技能、按移动落点复用最近敌距、以单遍择优替代完整排序，并移除绝技评分的高频效果数组分配。剩余热点是每个施法目标仍会创建影响格集合和目标列表、扫描全部战斗单位，且策略层仍要求完整物化候选集合。后续应先建立覆盖小型、常规和候选密集战场的固定 benchmark，再引入单次决策级 `BattleAiEvaluationContext`，集中维护存活单位/位置索引并让候选流式进入策略择优；不要引入跨战斗状态的全局可变缓存。需要用决策结果回归测试和分配量基准确保排序语义不变。
...
- 物品使用后续待建模：更完整的功能道具与普通剧情物品主动使用语义。战斗外已接入装备、武学书、绝技书、天赋书、属性/资源调整与剧情触发效果；消耗品已接入战斗内使用上下文、目标规则和效果执行。
- 战斗内用药冷却当前在选择目标并提交 `UseItem(...)` 后才返回失败提示。后续应从 Core 抽出供规则执行与 UI 共用的物品目标可用性查询：只能对自己用药且自身仍在冷却，或所有合法目标都不可用时，点击物品按钮便直接提示并保持当前交互状态；存在可用队友目标时仍允许打开物品面板，并在目标高亮中排除不可用角色。不要在 Godot 侧重复难度、冷却与天赋判断。
...
- 当前战斗运行时的技能使用经验与角色使用经验由宿主通过队伍判定限制为我方单位。后续如果支持网络对战，需要重新建模“哪一方可获得持久化成长”和服务端权威校验，避免客户端队伍归属或自动战斗流程导致成长写入错误。
- 当前战斗内角色升级直接修改 `CharacterInstance` 并通过 `BattleFactKind.CharacterLeveledUp` 做战斗内表现，没有发布应用层 `CharacterChangedEvent` / `CharacterLeveledUpEvent`。后续如果战斗中角色升级需要驱动 HUD、档案、成就或其他 session 订阅者，应把这条成长路径收敛回应用层服务或补明确的战斗结算事件桥接。
...
- 当前战斗运行时的命中结算暂按 legacy 顺序处理：目标侧 `BeforeHitResolved` 闪避/反制先落地，来源侧破闪避/失手后处理。因此可能出现“最终命中但目标闪避反制副作用已生效”的结果；后续需明确这是保留为原版结算顺序，还是拆成命中状态解析与最终结果副作用两阶段。
- 当前造成伤害的特殊技能通过 `ApplyDirectDamage` 同时绕过命中、普通攻防计算和 `BeforeDamageApplied`，把“独立伤害公式 / 无视护甲”与“必中 / 不可反弹 / 不可转移 / 不可吸收”错误绑定。后续收敛伤害管线时，特殊技能继续使用自身 Effect 与独立伤害公式，但应统一生成不可变的伤害描述并进入公共结算管线；伤害描述用强类型字段分别表达命中方式（判定命中/必定命中）、减伤方式（普通/无视护甲/真实伤害）、来源归因（技能/持续伤害/反弹/转移/施法代价）以及是否允许反弹、转移、分摊、吸收和触发受伤响应，内容 Tag 只用于能力匹配与扩展规则，不承载核心结算不变量。命中、伤害计算、承伤前反应、伤害分量落地、承伤后响应与战败阶段各自只解释相关字段；同归剑法、天地同寿的自伤应建模为不可防御的施法代价，火枪等无视护甲伤害仍可按技能语义参与闪避和目标侧反制，中毒等周期伤害则显式关闭不适用的阶段。不要继续按技能类别扩增 `ApplyDirectDamage`、`ApplyPoisonDamage` 等彼此割裂的伤害入口。
- 当前受击回怒采用“伤害事件级”语义，挂在公共 damage resolver 的 after-damage 路径：普通技能、直接/真实伤害和多段追加伤害只要造成敌对 HP 损失，都会各自按 `50% + 福缘 / 1000` 独立判定；显式关闭 after-damage 响应的 Buff 周期伤害不会判定。后续需明确继续保留事件级语义，还是改成“每次主动技能/攻击最多判定一次”；若改为后者，应引入伤害来源分类与攻击级结算上下文统一聚合，不在具体技能或天赋处理器中按偶然信息去重。
